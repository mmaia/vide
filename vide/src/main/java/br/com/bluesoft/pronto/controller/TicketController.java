...
public class TicketController {
	...
	private Logger log = Logger.getLogger(TicketController.class.getName());
	...
	public String listarPorBacklog(final Model model, final int backlogKey)
		...
		final List<Ticket> tickets = ticketDao
				.listarEstoriasEDefeitosPorBacklog(backlogKey);
		final List<Ticket> tarefasSoltas = ticketDao
				.listarTarefasEmBacklogsDiferentesDasEstoriasPorBacklog(backlogKey);

		model.addAttribute("tickets", tickets);
		model.addAttribute("tarefasSoltas", tarefasSoltas);
		model.addAttribute("backlog", backlogDao.obter(backlogKey));

		final Map<Integer, Integer> totaisPorTipoDeTicket = totaisPorTipoDeTicket(
				tickets, tarefasSoltas);
		model.addAttribute("descricaoTotal",
				montaDescricaoTotal(totaisPorTipoDeTicket));
	...
	public String listarPorSprint(final Model model, final int sprintKey)
		...
		final List<Ticket> tickets = ticketDao
				.listarEstoriasEDefeitosPorSprint(sprintKey);
		model.addAttribute("tickets", tickets);
	...
	public String listarTicketsPendentesPorCliente(final Model model,
			final Integer kanbanStatusKey, final Integer clienteKey,
			final String ordem, final String classificacao)
			throws SegurancaException {
		...
		tickets = ticketDao.buscar(null, kanbanStatusKey, clienteKey,
				ticketOrdem, ticketClassificacao);

		final Multimap<String, Ticket> ticketsAgrupados = ArrayListMultimap
				.create();
		for (final Ticket ticket : tickets) {
			// N�o exibe nem as tarefas nem os itens na lixeira
			if (ticket.getPai() == null
					&& ticket.getBacklog().getBacklogKey() != Backlog.LIXEIRA) {
				ticketsAgrupados.put(ticket.getCliente() != null ? ticket
						.getCliente().getNome() : "Indefinido", ticket);
			}
		}

		model.addAttribute("ticketsAgrupados", ticketsAgrupados.asMap());

		final ArrayList<String> grupo = new ArrayList<String>(ticketsAgrupados
				.keySet());
	...
	private String montaDescricaoTotal(
			final Map<Integer, Integer> totaisPorTipoDeTicket)
			throws SegurancaException {

		final Integer totalDeDefeitos = totaisPorTipoDeTicket
				.get(TipoDeTicket.DEFEITO);
		final Integer totalDeEstorias = totaisPorTipoDeTicket
				.get(TipoDeTicket.ESTORIA);
		final Integer totalDeTarefas = totaisPorTipoDeTicket
				.get(TipoDeTicket.TAREFA);
		final Integer totalDeIdeias = totaisPorTipoDeTicket
				.get(TipoDeTicket.IDEIA);

		final StringBuilder descricaoTotal = new StringBuilder();
		descricaoTotal.append(totalDeDefeitos > 0 ? totalDeDefeitos
				+ " defeito(s), " : "nenhum defeito, ");
		descricaoTotal.append(totalDeEstorias > 0 ? totalDeEstorias
				+ " projeto(s), " : "nenhum projeto, ");
		descricaoTotal.append(totalDeTarefas > 0 ? totalDeTarefas
				+ " tarefa(s), " : "nenhuma tarefa, ");
		descricaoTotal.append(totalDeIdeias > 0 ? totalDeIdeias + " ideia(s)"
				: "nenhuma ideia");

		return descricaoTotal.toString();
	}

	private Map<Integer, Integer> totaisPorTipoDeTicket(
			final List<Ticket>... listas) {
		...
		for (final List<Ticket> tickets : listas) {
			for (final Ticket ticket : tickets) {
				totais.put(ticket.getTipoDeTicket().getTipoDeTicketKey(),
						totais.get(ticket.getTipoDeTicket()
								.getTipoDeTicketKey()) + 1);
				for (final Ticket tarefa : ticket.getFilhos()) {
					if (tarefa.getBacklog().getBacklogKey() == ticket
							.getBacklog().getBacklogKey()) {
						totais.put(tarefa.getTipoDeTicket()
								.getTipoDeTicketKey(), totais.get(tarefa
								.getTipoDeTicket().getTipoDeTicketKey()) + 1);
					}
				}
			}
		}
	...
	public String branches(final Model model) {
		final List<Ticket> tickets = ticketDao
				.listarTicketsQueNaoEstaoNoBranchMaster();
		model.addAttribute("tickets", tickets);
	...
	public String sprintAtual(final Model model) {
		...
		if (quantidadeDeSprints == 0) {
			...
			model.addAttribute("mensagem",
					"Por favor, informe qual &eacute; o Sprint atual.");
			return "forward:/sprint/listar.action";
		} else {
			return "forward:/ticket/listarPorSprint.action?sprintKey="
					+ sprint.getSprintKey();
		}
	}
	...
	public String salvar(final Model model, final Ticket ticket,
			final String comentario, final String[] desenvolvedor,
			final String[] testador, final Integer paiKey,
			final Integer clienteKey) throws SegurancaException {
		log.info("Entrou TicketController.salvar");
		Seguranca.validarPermissao(Papel.PRODUCT_OWNER, Papel.EQUIPE,
				Papel.SCRUM_MASTER);
		Transaction tx = null;
		try {
			tx = sessionFactory.getCurrentSession()
					.beginTransaction();
			log.info("Iniciou transacao para gravar ticket");
			if (ticket.getTitulo() == null
					|| ticket.getTitulo().trim().length() <= 0) {
				throw new ProntoException(
						"N&atilde;o &eacute; poss&iacute;vel salvar um projeto, defeito ou tarefa sem t&iacute;tulo!");
			}

			if (paiKey == null) {
				log.info("Ticket e um projeto(nao tem pai)");
				ticket.setBacklog(backlogDao.obter(ticket.getBacklog()
						.getBacklogKey()));
				ticket.setTipoDeTicket(tipoDeTicketDao.obter(ticket
						.getTipoDeTicket().getTipoDeTicketKey()));

				addSprintToTicket(ticket);

			} else {
				log.info("Ticket nao e projeto(tem pai)");
				final Ticket pai = ticketDao.obter(paiKey);
				copiarDadosDoPai(pai, ticket);
			}

			ticket.setKanbanStatus(kanbanStatusDao.obter(ticket
					.getKanbanStatus().getKanbanStatusKey()));
			log.info("setou status no kanban ==>> " + ticket.getKanbanStatus());
			if (clienteKey != null) {
				ticket.setCliente(clienteDao.obter(clienteKey));
				log.info("Setou cliente no ticket");
			}

			if (ticket.getTicketKey() == 0) {
				ticket.setReporter(usuarioDao.obter(ticket.getReporter()
						.getUsername()));
				log.info("Setou reporter do ticket(quem editou)");
			}

			if (comentario != null && comentario.trim().length() > 0) {
				ticket.addComentario(comentario, Seguranca.getUsuario());
				log.info("Setou comentario do ticket");
			}

			definirDesenvolvedores(ticket, desenvolvedor);
			log.info("Setou desenvolvedores");
			definirTestadores(ticket, testador);
			log.info("Setou testadores");

			ticketDao.salvar(ticket);

			tx.commit();
			log.info("Executou TicketController.salvar");
			return "redirect:editar.action?ticketKey=" + ticket.getTicketKey();
		} catch (final Exception e) {
			e.printStackTrace();
			model.addAttribute("erro", e.getMessage());
			log.info("ERRO - TicketController.salvar!");
			log.error("Ooooopssss, ocorreu um erro vou dar rollback na transacao! ==>> " + e.getMessage());
			tx.rollback();
	...
	private void addSprintToTicket(final Ticket ticket) {
		log.info("Entrou TicketController.addSprintToTicket");
		if (!ticket.isTarefa() && ticket.getSprint() != null
				&& ticket.getSprint().getSprintKey() <= 0) {
			ticket.setSprint(null);
			log.info("Sprint foi setado para null no ticket..");
		} 
		else if (ticket.isTarefa() && ticket.getSprint() != null && ticket.getSprint().getSprintKey() <= 0) {
			log.info("Ticket.isTarefa recuperando sprint do pai para cadastro da tarefa.");
			ticket.setSprint(ticket.getPai().getSprint());
		} 
		else {
			log.info("Setando sprint pegando valor da combo na pagina");
			ticket.setSprint((Sprint) sessionFactory.getCurrentSession().get(
					Sprint.class, ticket.getSprint().getSprintKey()));
		}
		log.info("Executou TicketController.addSprintToTicket");
	}

	private void definirDesenvolvedores(final Ticket ticket,
			final String[] desenvolvedor) throws SegurancaException {

		final Set<Usuario> desenvolvedoresAntigos = new TreeSet<Usuario>(
				ticketDao.listarDesenvolvedoresDoTicket(ticket.getTicketKey()));
		...
		if (desenvolvedor != null && desenvolvedor.length > 0) {
			...
			for (final String username : desenvolvedor) {
				ticket.addDesenvolvedor((Usuario) sessionFactory
						.getCurrentSession().get(Usuario.class, username));
			}
		}

		final String desenvolvedoresAntigosStr = desenvolvedoresAntigos == null
				|| desenvolvedoresAntigos.size() == 0 ? "nenhum"
				: desenvolvedoresAntigos.toString();
		final String desenvolvedoresNovosStr = ticket.getDesenvolvedores() == null
				|| ticket.getDesenvolvedores().size() == 0 ? "nenhum" : ticket
				.getDesenvolvedores().toString();
		if (!desenvolvedoresAntigosStr.equals(desenvolvedoresNovosStr)) {
			ticket.addLogDeAlteracao("desenvolvedores",
					desenvolvedoresAntigosStr, desenvolvedoresNovosStr);
		}
	}
	...
	private void definirTestadores(final Ticket ticket, final String[] testador)
		...
		final Set<Usuario> testadoresAntigos = new TreeSet<Usuario>(ticketDao
				.listarTestadoresDoTicket(ticket.getTicketKey()));
		...
		final String testadoresAntigosStr = testadoresAntigos == null
				|| testadoresAntigos.size() == 0 ? "nenhum" : testadoresAntigos
				.toString();
		final String testadoresNovosStr = ticket.getTestadores() == null
				|| ticket.getTestadores().size() == 0 ? "nenhum" : ticket
				.getTestadores().toString();
		if (!testadoresAntigosStr.equals(testadoresNovosStr)) {
			ticket.addLogDeAlteracao("testadores", testadoresAntigosStr,
					testadoresNovosStr);
		}
	}

	@RequestMapping("/ticket/jogarNoLixo.action")
	public String jogarNoLixo(final Model model, final int ticketKey,
			final HttpServletResponse response) throws SegurancaException {

		Seguranca.validarPermissao(Papel.PRODUCT_OWNER, Papel.EQUIPE);

		final Ticket ticket = (Ticket) sessionFactory.getCurrentSession().get(
				Ticket.class, ticketKey);
		ticket.setBacklog((Backlog) sessionFactory.getCurrentSession().get(
				Backlog.class, Backlog.LIXEIRA));
		ticketDao.salvar(ticket);
		return "redirect:/ticket/editar.action?ticketKey=" + ticketKey;
	}

	@RequestMapping("/ticket/moverParaImpedimentos.action")
	public String moverParaImpedimentos(final Model model, final int ticketKey,
			final HttpServletResponse response) throws SegurancaException {
		...
		final Ticket ticket = (Ticket) sessionFactory.getCurrentSession().get(
				Ticket.class, ticketKey);
		ticket.setBacklog((Backlog) sessionFactory.getCurrentSession().get(
				Backlog.class, Backlog.IMPEDIMENTOS));
		ticketDao.salvar(ticket);
		return "redirect:/ticket/editar.action?ticketKey=" + ticketKey;
	}

	@RequestMapping("/ticket/moverParaBranchMaster.action")
	public String moverParaBranchMaster(final Model model, final int ticketKey,
			final HttpServletResponse response) throws SegurancaException {
	...
	public String moverParaProductBacklog(final Model model,
			final int ticketKey, final HttpServletResponse response)
			throws SegurancaException {
		...
		ticket.setBacklog((Backlog) sessionFactory.getCurrentSession().get(
				Backlog.class, Backlog.PRODUCT_BACKLOG));

		// caso n�o seja uma ideia mant�m o tipo original
		if (ticket.getTipoDeTicket().getTipoDeTicketKey() == TipoDeTicket.IDEIA) {
			ticket.setTipoDeTicket((TipoDeTicket) sessionFactory
					.getCurrentSession().get(TipoDeTicket.class,
							TipoDeTicket.ESTORIA));
		}
	...
	public String moverParaOSprintAtual(final Model model, final int ticketKey,
			final HttpServletResponse response) throws ProntoException {
		...
		if (backlogDeOrigem != Backlog.PRODUCT_BACKLOG) {
			throw new ProntoException(
					"Para que um projeto ou Bug seja movida para o Sprint Atual &ecute; preciso que ela esteja no Product Backlog.");
		}
	...
	public String moverParaIdeias(final Model model, final int ticketKey,
			final HttpServletResponse response) throws SegurancaException {
		...
		final Ticket ticket = (Ticket) sessionFactory.getCurrentSession().get(
				Ticket.class, ticketKey);
		ticket.setBacklog((Backlog) sessionFactory.getCurrentSession().get(
				Backlog.class, Backlog.IDEIAS));
		ticket.setTipoDeTicket((TipoDeTicket) sessionFactory
				.getCurrentSession()
				.get(TipoDeTicket.class, TipoDeTicket.IDEIA));
		ticket.setSprint(null);
		ticketDao.salvar(ticket);
		return "redirect:/ticket/editar.action?ticketKey=" + ticketKey;
	}
	...
	public String restaurar(final Model model, final int ticketKey,
			final HttpServletResponse response) throws SegurancaException {
		...
		case TipoDeTicket.DEFEITO:
			backlog = (Backlog) sessionFactory.getCurrentSession().get(
					Backlog.class, Backlog.PRODUCT_BACKLOG);
			break;
	...
	public String upload(final HttpServletRequest request, final int ticketKey)
			throws Exception {
		...
		for (final FileItem fileItem : items) {
			final String nomeDoArquivo = fileItem.getName().toLowerCase()
					.replace(' ', '_').replaceAll("[^A-Za-z0-9._\\-]", "");
	...
	public String excluirAnexo(final String file, final int ticketKey)
			throws Exception {
		final File arquivo = new File(Config.getImagesFolder() + ticketKey
				+ "/" + file);
		if (arquivo.exists()) {
			arquivo.delete();
		}
	...
	public String download(final HttpServletResponse response,
			final String file, final int ticketKey) throws Exception {
		...
		if (extensao == null) {
			...
			mime = "text/plain";
				...
				|| extensao.equalsIgnoreCase("docx")) {
	...
	public String estimarSprint(final Model model, final int sprintKey)
		...
		final List<Ticket> tickets = ticketDao
				.listarEstoriasEDefeitosPorSprint(sprintKey);
		model.addAttribute("tickets", tickets);
		model.addAttribute("sprint", sessionFactory.getCurrentSession().get(
				Sprint.class, sprintKey));
		return VIEW_ESTIMAR;
	}
	...
	public String estimarBacklog(final Model model, final int backlogKey)
		...
		final List<Ticket> tickets = ticketDao
				.listarEstoriasEDefeitosPorBacklog(backlogKey);
		model.addAttribute("tickets", tickets);
		model.addAttribute("backlog", sessionFactory.getCurrentSession().get(
				Backlog.class, backlogKey));
		return VIEW_ESTIMAR;
	}

	@RequestMapping("/ticket/salvarValorDeNegocio.action")
	public void salvarValorDeNegocio(final HttpServletResponse response,
			final int ticketKey, final int valorDeNegocio)
			throws SegurancaException {
		try {
			Seguranca.validarPermissao(Papel.PRODUCT_OWNER);
	...
	public void salvarBranch(final HttpServletResponse response,
			final int ticketKey, final String branch) throws SegurancaException {
		try {
	...
	public void salvarEsforco(final HttpServletResponse response,
			final int ticketKey, final double esforco)
			throws SegurancaException {
		try {
			Seguranca.validarPermissao(Papel.EQUIPE);
			final Ticket ticket = ticketDao.obter(ticketKey);
	...
	public void salvarPar(final HttpServletResponse response,
			final int ticketKey, final boolean par) throws SegurancaException {
		try {
			Seguranca.validarPermissao(Papel.EQUIPE);
			final Ticket ticket = ticketDao.obter(ticketKey);
			ticket.setPar(par);
	...
	public String listarTarefasParaAdicionarAoSprint(final Model model,
			final int sprintKey) throws SegurancaException {

		Seguranca.validarPermissao(Papel.PRODUCT_OWNER, Papel.EQUIPE);

		model.addAttribute("sprint", sessionFactory.getCurrentSession().get(
				Sprint.class, sprintKey));
		model.addAttribute("tickets", ticketDao
				.listarEstoriasEDefeitosDoProductBacklog());
		return "/ticket/ticket.adicionarAoSprint.jsp";
	}

	@RequestMapping("/ticket/adicionarAoSprint.action")
	public String adicionarAoSprint(final Model model, final int sprintKey,
			final int[] ticketKey) {

		final Sprint sprint = (Sprint) sessionFactory.getCurrentSession().get(
				Sprint.class, sprintKey);
		for (final int key : ticketKey) {
			final Ticket ticket = (Ticket) sessionFactory.getCurrentSession()
					.get(Ticket.class, key);
			ticket.setBacklog((Backlog) sessionFactory.getCurrentSession().get(
					Backlog.class, Backlog.SPRINT_BACKLOG));
	...
	public String logDescricao(final Model model, final int ticketHistoryKey) {
		...
		final TicketLog log = (TicketLog) sessionFactory.getCurrentSession()
				.get(TicketLog.class, ticketHistoryKey);
	...
	public String transformarEmEstoria(final Model model, final int ticketKey)
		...
		final Ticket ticket = ticketDao.obter(ticketKey);
		ticket.setTipoDeTicket((TipoDeTicket) sessionFactory
				.getCurrentSession().createCriteria(TipoDeTicket.class).add(
						Restrictions
								.eq("tipoDeTicketKey", TipoDeTicket.ESTORIA))
				.uniqueResult());
		ticketDao.salvar(ticket);
	...
	public String transformarEmDefeito(final Model model, final int ticketKey)
		...
		if (ticket.getFilhos() != null && ticket.getFilhos().size() > 0) {
			model
					.addAttribute(
							"erro",
							"Esse projeto possui tarefas e por isso n&atilde;o pode ser transformada em um defeito.");
		} else {
			ticket.setTipoDeTicket((TipoDeTicket) sessionFactory
					.getCurrentSession().createCriteria(TipoDeTicket.class)
					.add(
							Restrictions.eq("tipoDeTicketKey",
									TipoDeTicket.DEFEITO)).uniqueResult());
			ticketDao.salvar(ticket);
		}
		return "forward:/ticket/editar.action";
	}
	...
	public String verDescricao(final Model model, final int ticketKey)
			throws Exception {
		final Ticket ticket = ticketDao.obter(ticketKey);
	...
	public String editar(final Model model, final Integer ticketKey,
			final Integer tipoDeTicketKey, final Integer backlogKey)
			throws SegurancaException {
		log.info("Entrou TicketController.editar");
		Seguranca.validarPermissao(Papel.PRODUCT_OWNER, Papel.EQUIPE);
		...
		if (ticketKey != null) {
			...
			if (ticket == null) {
				model.addAttribute("mensagem", "O Ticket #" + ticketKey
						+ " n&atilde;o existe.");
				return "/branca.jsp";
			}
			model.addAttribute("sprints", sprintDao.listarSprintsEmAberto());
			...
			novoTicket.setTipoDeTicket((TipoDeTicket) sessionFactory
					.getCurrentSession().get(TipoDeTicket.class,
							tipoDeTicketKey));
			novoTicket.setBacklog((Backlog) sessionFactory.getCurrentSession()
					.get(Backlog.class, backlogKey));
		...
		model.addAttribute("kanbanStatus", kanbanStatusDao.listar());
		log.info("Executou TicketController.editar");
		return VIEW_EDITAR;
	}

	@RequestMapping("/ticket/incluirTarefa.action")
	public String incluirTarefa(final Model model, final int paiKey)
			throws Exception {
		...
		tarefa.setSprint(pai.getSprint());
	...
	private void copiarDadosDoPai(final Ticket pai, final Ticket filho) {
		log.info("Entrou TicketController.copiarDadosDoPai");
		filho.setPai(pai);
		if (filho.getReporter() == null
				|| filho.getReporter().getUsername() == null) {
